...
def scatter_format(PARAM,Title,i,j,X,Y,COLOR,ALPHA,LABEL,MARKER,LS,Name_Xaxis,Name_Yaxis,Xmin,Xmax,Xinterval,Ymin,Ymax,Yinterval,xLOG,yLOG,Grid):
    ...
    #LS#Line Style: list length is j
    ...
    if(i<=1):
        for ii in range(i):
            ...
            Yi=Y[ii]
            for jj in range(j):
                plt.plot(x=Xi[jj],y=Yi[jj],c=COLOR[jj],maker=MARKER[jj],alpha=ALPHA[jj],ls=LS[jj],label=LABEL[jj])
